          ...
          <div class="box-body">
            <div class="form-group">
              ...
              <select name="ddlVDate" id="ddlVDate" class="form-control">
                ...
                <?php
                $result_floor = mysqli_query($link, "SELECT * FROM tbl_add_complain order by complain_id ASC");
                while ($row_floor = mysqli_fetch_array($result_floor)) { ?>
                  <option <?php if ($v_date == $row_floor['c_date']) {
                            echo 'selected';
                          } ?> value="<?php echo $row_floor['c_date']; ?>"><?php echo $row_floor['c_date']; ?></option>
                <?php } ?>
              </select>
            </div>
            <div class="form-group">
    ...
    function getVisitorsInfo() {
      ...
      if (v_date != '' && month_id != '' && xyear != '') {
        //window.location = "<?php //echo WEB_URL;
                              ?>report/mark_info.php?cid=" + class_id + '&eid=' + exam_id + '&sbid=' + subject_id;
